...
public interface FunctionDynamicCallFunctionRepository extends Neo4jRepository<DynamicCall, Long> {
	 ...
	 * @return
	 */
	@Query("match (a:Function)-[r:" + RelationType.str_DYNAMIC_CALL + "]-(b:Function) where id(a) = {0} return b")
	...
	@Query("MATCH result=(function1:Function)-[r:" + RelationType.str_DYNAMIC_CALL + "]->(function2:Function) with function1,function2,result match (project:Project)-[r2:" + RelationType.str_CONTAIN + "*3..5]->(function1) where id(project)={projectId} RETURN result")
	public List<DynamicCall> findProjectContainFunctionDynamicCallFunctionRelations(@Param("projectId") Long projectId);

	@Query("match result = (f1:Function)-[r1:" + RelationType.str_CALL + "]->(f2:Function) where not (f1)-[:" + RelationType.str_DYNAMIC_CALL + "{testCaseId:{testcaseId}}]->(f2) return result")
	public List<Call> findFunctionCallFunctionNotDynamicCalled(@Param("testcaseId") Integer testcaseId);
	
	@Query("match result = (f1:Function)-[r1:" + RelationType.str_CALL + "]->(f2:Function) where (f1)-[:" + RelationType.str_DYNAMIC_CALL + "{testCaseId:{testcaseId}}]->(f2) return result")
	public List<Call> findFunctionCallFunctionDynamicCalled(@Param("testcaseId") Integer testcaseId);
	
	@Query("match result = (f1:Function)-[r1:" + RelationType.str_CALL + "]->(f2:Function) where not (f1)-[:" + RelationType.str_DYNAMIC_CALL + "]->(f2) return result")
	public List<Call> findFunctionCallFunctionNotDynamicCalled();
	
	@Query("MATCH result=(function1:Function)-[r:" + RelationType.str_DYNAMIC_CALL + "]->(function2:Function) where id(function1)={functionId} return result")
	...
	@Query("MATCH result=(function1:Function)-[r:" + RelationType.str_DYNAMIC_CALL + "]->(function2:Function) where id(function1)={callerId1} and id(function2)={calledId} return result")
