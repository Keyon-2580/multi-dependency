...
public class JavaStubListenerUsingTryFinallyForTestCase extends JavaStubListenerUsingTryFinally {
	...
	public JavaStubListenerUsingTryFinallyForTestCase(TokenStream tokens, String projectName, File listenFile,
	...
	public void enterClassBodyDeclaration(ClassBodyDeclarationContext ctx) {
		...
		for(ModifierContext modifier : modifiers) {
			...
			if(lastAnnotationName.equals("Test")) {
				isTest = true;
				break;
			}
		}
	...
	protected String startMethod(String methodName, List<String> parameterNames) {
		...
		if(isTest) {
			builder
			.append(MULTIPLE_STUB_VARIABLE_MAP_TRACE_ID).append(".put(java.lang.Thread.currentThread().getId(), \"").append(getMethodFullName(methodName, parameterNames)).append("\");")
			.append(MULTIPLE_STUB_VARIABLE_MAP_SPAN_ID).append(".put(java.lang.Thread.currentThread().getId(), \"").append(getMethodFullName(methodName, parameterNames)).append("\");")
